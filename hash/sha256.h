...
// Función que calcula el HMAC utilizando SHA-256
std::string calculateSHA256HMAC(const std::string& message) {
    ...
    SHA256(reinterpret_cast<const unsigned char*>(concatKeyMessage.c_str()), concatKeyMessage.length(), messageHash);
    ...
    unsigned char finalResult[SHA256_DIGEST_LENGTH];
    SHA256(reinterpret_cast<const unsigned char*>(finalHash.c_str()), finalHash.length(), finalResult);
